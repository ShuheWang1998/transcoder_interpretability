...
tokenized_owt = tokenized_owt.take(256)

owt_tokens = np.stack([x['tokens'] for x in tokenized_owt])

owt_tokens_torch = torch.from_numpy(owt_tokens).cuda()

print(owt_tokens_torch.shape)
...
transcoders.append(SparseAutoencoder.load_from_pretrained(f"{transcoder_template.format(0)}.pt").eval())
frequencies.append(torch.load(f"{transcoder_template.format(0)}_log_feature_sparsity.pt"))
...
scores = get_feature_scores(model, transcoders[0], owt_tokens_torch, 0, batch_size=128)
